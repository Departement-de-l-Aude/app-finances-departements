...
def generer_graphiques(df_plot, titre, indicateurs, par_habitant=False, afficher_les_deux=False, superposer=False):
    ...
    for i, axe_indice_i in enumerate(axes_liste[:n]):    # On met un coup de slicing pour eviter de faire un tour en trop quand on a un nombre impairs d'indicateur (cf le graphique vide de fin)
        ...
        if superposer:
            ...
            sns.lineplot(data=df_plot, x="Exercice", y=indic, hue="Nom " + str(annee_max) + " Département", style="Nom " + str(annee_max) + " Département", markers=True, dashes=False, ax=axe_indice_i, linewidth=3)
...
def analyser_un_departement(df_arg, code_dep, intervalle_annees, indicateurs, par_habitant=False, afficher_les_deux=False):
    df_temp = df_arg.copy()
    df_temp["Code Insee " + str(annee_max) + " Département"] = df_temp["Code Insee " + str(annee_max) + " Département"].astype(str)
    code_dep = str(code_dep)
    annee_min_temp, annee_max_temp = intervalle_annees
    
    serie_filtre = (df_temp["Type de budget"] == "Budget principal") & (df_temp["Code Insee " + str(annee_max) + " Département"] == code_dep) & (annee_min_temp <= df_temp["Exercice"]) & (df_temp["Exercice"] <= annee_max_temp)
                    
    index_colonnes = ["Exercice", "Nom " + str(annee_max) + " Département", "Population totale"]
    ...
    nom_dep = pivot["Nom " + str(annee_max) + " Département"].iloc[0]    # On récupère le nom du département pour plus tard l'afficher et pas avoir que les numéros de départements
    ...
    colonnes_utiles = ["Exercice", "Nom " + str(annee_max) + " Département"] + indicateurs_a_tracer
...
def departements_meme_strate(df_arg, code_dep, mm_region=False):
    ...
    df_dep_cible = df_temp[df_temp["Code Insee " + str(annee_max) + " Département"] == code_dep]
    if df_dep_cible.empty: 
        return pd.DataFrame()

    strate = df_dep_cible["Strate population " + str(annee_max) + ""].iloc[0]
    region = df_dep_cible["Nom " + str(annee_max) + " Région"].iloc[0]
    
    serie_filtre = df_temp["Strate population " + str(annee_max) + ""] == strate
    if mm_region:
        serie_filtre = serie_filtre & (df_temp["Nom " + str(annee_max) + " Région"] == region)

    df_resultat = df_temp.loc[serie_filtre, ["Code Insee " + str(annee_max) + " Département", "Nom " + str(annee_max) + " Département", "Nom " + str(annee_max) + " Région"]].drop_duplicates()
    df_resultat = df_resultat[df_resultat["Code Insee " + str(annee_max) + " Département"] != code_dep] # Ici on exclut notre département de la liste
...
def comparer_departements(df_arg, liste_codes_dep, intervalle_annees, indicateurs, par_habitant=False, afficher_les_deux=False):
    ...
    serie_filtre = (df_temp["Type de budget"] == "Budget principal") & (df_temp["Code Insee " + str(annee_max) + " Département"].isin(liste_codes_dep)) & (annee_min_temp <= df_temp["Exercice"]) & (df_temp["Exercice"] <= annee_max_temp)
    ...
    colonnes_utiles = ["Exercice", "Nom " + str(annee_max) + " Département"] + indicateurs_a_tracer
    df_final = pivot[[colonne for colonne in colonnes_utiles if colonne in pivot.columns]].round(1).sort_values(by=["Exercice", "Nom " + str(annee_max) + " Département"])
    
    return fig, df_final


def comparer_departement_strate(df_arg, code_dep, intervalle_annees, indicateurs, afficher_metro=True, afficher_region=False, par_habitant=False, afficher_les_deux=False):
    df_temp = df_arg.copy()
    df_temp["Code Insee " + str(annee_max) + " Département"] = df_temp["Code Insee " + str(annee_max) + " Département"].astype(str)
    code_dep = str(code_dep)
    annee_min_temp, annee_max_temp = intervalle_annees
    ...
    strate = df_dep_cible["Strate population " + str(annee_max) + ""].iloc[0]
    nom_dep = df_dep_cible["Nom " + str(annee_max) + " Département"].iloc[0]
    region = df_dep_cible["Nom " + str(annee_max) + " Région"].iloc[0]
    
    serie_filtre = (df_temp["Type de budget"] == "Budget principal") & (df_temp["Strate population " + str(annee_max) + ""] == strate) & (annee_min_temp <= df_temp["Exercice"]) & (df_temp["Exercice"] <= annee_max_temp)
                    
    index_colonnes = ["Exercice", "Code Insee " + str(annee_max) + " Département", "Nom " + str(annee_max) + " Département", "Nom " + str(annee_max) + " Région", "Outre-mer", "Population totale"]
    ...
    df_cible = pivot[pivot["Code Insee " + str(annee_max) + " Département"] == code_dep].copy()
    df_autres = pivot[(pivot["Code Insee " + str(annee_max) + " Département"] != code_dep) & (pivot["Outre-mer"] == "Non")].copy() # On ne prend pas en compte notre département dans la moyenne et on ne prend pas les outre-mer
    ...
    if afficher_metro and not df_autres.empty:
        df_moyenne_metro = df_autres.groupby("Exercice")[cols_mean].mean().reset_index()
        df_moyenne_metro["Nom " + str(annee_max) + " Département"] = f"Moyenne strate {strate} (métropole)"
        list_df_to_concat.append(df_moyenne_metro)
        
    if afficher_region and not df_autres.empty:
        df_autres_region = df_autres[df_autres["Nom " + str(annee_max) + " Région"] == region]
        if not df_autres_region.empty:
            df_moyenne_region = df_autres_region.groupby("Exercice")[cols_mean].mean().reset_index()
            df_moyenne_region["Nom " + str(annee_max) + " Département"] = f"Moyenne strate {strate} (même région)"
    ...
    colonnes_utiles = ["Exercice", "Nom " + str(annee_max) + " Département"] + indicateurs_a_tracer
    df_final = df_plot[[colonne for colonne in colonnes_utiles if colonne in df_plot.columns]].round(1).sort_values(by=["Exercice", "Nom " + str(annee_max) + " Département"])
...
def comparer_departement_strate_metro(df_arg, code_dep, intervalle_annees, indicateurs, meme_region=False, par_habitant=False, afficher_les_deux=False):
    ...
    serie_filtre = (df_temp["Type de budget"] == "Budget principal") & ((df_temp["Outre-mer"] == "Non") | (df_temp["Code Insee " + str(annee_max) + " Département"] == code_dep)) & (annee_min_temp <= df_temp["Exercice"]) & (df_temp["Exercice"] <= annee_max_temp)
                    
    index_colonnes = ["Exercice", "Code Insee " + str(annee_max) + " Département", "Nom " + str(annee_max) + " Département", "Strate population " + str(annee_max) + "", "Outre-mer", "Nom " + str(annee_max) + " Région", "Population totale"]
    ...
    df_strate = pivot[(pivot["Strate population " + str(annee_max) + ""] == strate) & (pivot["Code Insee " + str(annee_max) + " Département"] != code_dep)].copy()
    
    if meme_region:
        df_strate = df_strate[df_strate["Nom " + str(annee_max) + " Région"] == region]

    df_moy_strate = df_strate.groupby("Exercice")[cols_mean].mean().reset_index()
    label_moyenne = f"Moyenne strate {strate}" + (" (même région)" if meme_region else " (métropole)")
    df_moy_strate["Nom " + str(annee_max) + " Département"] = label_moyenne

    df_metro = pivot[pivot["Outre-mer"] == "Non"].copy()
    df_moy_metro = df_metro.groupby("Exercice")[cols_mean].mean().reset_index()
    df_moy_metro["Nom " + str(annee_max) + " Département"] = "Moyenne métropole"
...
liste_deps = sorted(df["Code Insee " + str(annee_max) + " Département"].astype(str).unique())
...
if menu == "Comparaison d'indicateurs d'un même département (1 seul graphique affiché)":
    ...
    with col2:
        st.write("")
        st.write("")
        afficher_metro = st.checkbox("Afficher la moyenne de la strate (métropole)", value=True)
    ...
    if st.button("Générer l'analyse"):
        if not afficher_metro and not afficher_region:
            st.error("⚠️ Veuillez cocher au moins une moyenne à afficher (métropole et/ou même région).")
        else:
            fig, data = comparer_departement_strate(df, dep, annees_sel, indicateurs_choisis, afficher_metro, afficher_region, par_habitant, afficher_les_deux)
            if fig:    
                buf = io.BytesIO()
                fig.savefig(buf, format="pdf", bbox_inches="tight")
                st.download_button(
