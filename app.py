...
def analyser_un_departement(df_arg, code_dep, intervalle_annees, indicateurs, par_habitant=False, afficher_les_deux=False):
    ...
    if afficher_les_deux:                                                                                                                           
        ...
        ax1.set_xlim(pivot["Exercice"].min(), pivot["Exercice"].max())    # On force l'échelle sur l'axe des abscisses pour éviter que le code crash 
        ax2.set_xlim(pivot["Exercice"].min(), pivot["Exercice"].max())    # (quand il n'y a pas de données à tracer par exemple)
